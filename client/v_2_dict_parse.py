...
def food_parse(food_results, nutrient_dict, values):
    ...
    for food in food_results:
        ...
        for nutrient in food["foodNutrients"]:
            ...
            #nutrient['nutrient']['number'] = str(nutrient['nutrient']['number'])
        ...
        for nutrient in food["foodNutrients"]:
            ...
            #if (nutrient["nutrient"]['number']) == str(tracked_nutrients[n]):
            ...
            # found a number 205.2
            #while n < len(tracked_nutrients) and float(nutrient["nutrient"]["number"]) > tracked_nutrients[n]:
            while n < len(tracked_nutrients) and (nutrient["nutrient"]["number"]) > tracked_nutrients[n]:
                ...
                # to_insert is different from present format
                # and the code further down fails -> making it
                # in the format so that below code doesn't break
